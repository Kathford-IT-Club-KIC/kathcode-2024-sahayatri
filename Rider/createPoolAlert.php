                    ...
                    if (data.length > 0) {
                        ...
                        data.forEach((request, i) => {
                            ...
                            acceptButton.onclick = function() {
                                ...
                                .then(result => {
                                    if (result.success) {
                                        window.location.href = `getALertDetails.php?alertId=${alertId}&status=booked`;
                                    } else {
                ...
                if (status === 'OK') {
                    ...
                    var processRequest = function(index) {
                        if (index >= poolRequests.length) {
                            ...
                            if (requestsWithOverlap.length === 0) {
                                // Display no routes found message
                                var noRoutesCard = document.createElement('div');
                                noRoutesCard.className = 'card bg-gray-900 text-white p-4 rounded-lg shadow-md';
                                noRoutesCard.innerText = 'No routes found';
                                cardsContainer.appendChild(noRoutesCard);
                            } else {
