...
$sqlBooked = "SELECT * FROM poolrequests WHERE status = 'booked' AND userId= $userId";
...
$sqlAvailable = "SELECT * FROM poolrequests WHERE status = 'available' AND userId= $userId";
...
$sqlCompleted = "SELECT * FROM poolmappings INNER JOIN poolrequests ON poolmappings.poolRequestId = poolrequests.id WHERE poolmappings.completed = 'yes' AND poolRequests.userId=$userId";
...
<script>
    ...
    function navigateToDetails(status, requestId) {
        if (status === 'booked') {
            var url = `getRequestDetails.php?requestId=${requestId}&status=booked`;
        } else if (status === 'pending') {
            var url = `poolMapDetail.php?id=${requestId}&type=request`;
        } else if (status === 'completed') {
            var url = `getRequestDetails.php?requestId=${requestId}&status=completed`;
        }
